...
"""This file contains the tools used by the MySQL database agent."""
...
MYSQL_TOOLSET = os.getenv("MYSQL_TOOLSET", "mysql-database-tools")
MYSQL_SERVER_URL = os.getenv("MYSQL_SERVER_URL", "http://127.0.0.1:5000")
...
def get_toolbox_client():
    ...
    if toolbox_client is None:
        toolbox_client = ToolboxSyncClient(MYSQL_SERVER_URL)
...
def get_toolbox_toolset():
    ...
    if toolbox_toolset is None:
        toolbox_toolset = get_toolbox_client().load_toolset(MYSQL_TOOLSET)
...
def get_schema():
    ...
    #schema = get_schema_tool(get_env_var("MYSQL_SCHEMA_NAME"))
...
def update_database_settings():
    ...
    database_settings = {
        "project_id": get_env_var("MYSQL_PROJECT_ID"),
        "database": get_env_var("MYSQL_DATABASE"),
        "schema_name": get_env_var("MYSQL_SCHEMA_NAME"),
        "schema": schema,
    }
    return database_settings
